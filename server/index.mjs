...
export const createApp = async ({ serveStatic = true, enableLocalUploads = serveStatic } = {}) => {
  ...
  const appPromise = (async () => {
    ...
    app.get('/api/export-stats', async (request, response) => {
      try {
        const stats = await listExportStats(request.app.locals.db);
        response.json({ stats });
      } catch (error) {
        response.status(500).json({ error: error instanceof Error ? error.message : 'Failed to load export statistics.' });
      }
    });
    ...
    app.post('/api/admin/export-stats/upsert', requireAdmin, async (request, response) => {
      try {
        const stat = request.body?.stat;
        if (!stat || typeof stat !== 'object') {
          response.status(400).json({ error: 'Statistic data is required.' });
          return;
        }

        const savedStat = await upsertExportStat(request.app.locals.db, stat);
        void syncDatabaseToStaticJson(request.app.locals.db, projectRoot);
        response.json({ ok: true, stat: savedStat });
      } catch (error) {
        response.status(400).json({ error: error instanceof Error ? error.message : 'Failed to save export statistic.' });
      }
    });

    app.delete('/api/admin/export-stats/:id', requireAdmin, async (request, response) => {
      try {
        await deleteExportStatById(request.app.locals.db, request.params.id);
        void syncDatabaseToStaticJson(request.app.locals.db, projectRoot);
        response.json({ ok: true });
      } catch (error) {
